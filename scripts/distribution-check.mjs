// 人格分布检查工具(回归门禁,挂在 `npm run check` 里,勿删——删了 check 会直接失败)。
// 目的:验收标准是"最高人格占比 / 最低人格占比 ≤ 2.5",防止调 weights 后某人格几乎测不出来。
// 16 题全枚举有数十亿组合,不现实 → 用蒙特卡洛随机抽样估计分布(默认 50 万次,约 1.5s)。
// 计分逻辑与 lib/scoring.ts 保持一致(累加 weights,平分时按 personas 顺序取先者)。
// 达标退出码 0,偏斜退出码 1,可直接用于 CI/门禁。
