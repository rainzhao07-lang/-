...
一个移动端优先的付费测试H5「本命猫」：用户答基础题 → 计分引擎匹配出"铲屎官人格"与"本命猫品种" → 免费展示结果与可下载的分享卡 → 用户凭兑换码解锁 → 完成付费定制题 → 本地规则生成个性化"养猫决策报告"。
...
| 报告生成 | 本地规则 + 文案模块，根据基础答案、付费定制题、品种事实库和现实冲突生成，保持流式输出体验 |
...
微反馈机制：在第4题、第8题答完后，取当前累计得分最高人格的 `microFeedbackPool` 随机一句，以气泡动画插入（纯规则驱动，零成本零延迟）。

### 4.3 `/lib/local-report.ts` — 本地报告生成器

导出本地报告生成函数，注入：persona对象、用户答案摘要、hard_flags、premium_flags、品种事实库与现实冲突。生成结果必须包含以下硬规则：
...
2. 汇总所有 flags 为 hard_flags 对象，存入 session，并注入本地报告生成器（例如 shedding:"low" 时，报告须诚实提示布偶掉毛现实并给出打理方案——**硬条件不改变人格结果，但必须改变建议内容**）
...
### `/` 落地页
- 主标题：**「测测你内心住着哪只猫」**
- 副标题：你的人格猫，就是更契合你生活方式的那只。3分钟避开冲动养猫的坑。
...
| /api/report | POST | 入参 sessionId；校验 paid 和 premium_flags；查 reports 缓存，命中直接返回；未命中则调用本地报告生成器，流式返回并写缓存 |
...
- 仅服务端生成；不调用外部内容生成接口
- 保持流式输出体验，但内容来自规则和文案模块
- 同一 session **永不重复生成**（缓存优先），刷新页面直接命中缓存
- 输入防御：用户答案仅以选项下标和服务端题库摘要参与生成，不拼接任何用户自由文本（本产品无自由输入，保持这一点）
...
**M3 报告生成**
- [ ] /api/report 本地规则生成 + 缓存 + 打字机渲染；报告生成器含全部硬规则
...
- 运营者如何：替换 /content 内容、生成兑换码、调整本地报告规则、更换收款链接
